...
class RecurrentNeuralNetwork:
    ...
    def forwardProp(self):
        ...
        for i in range(1, self.rl + 1):
            ...
            self.x = self.eo[i - 1]  # set input to the next word in the sequence
        return self.oa
...
def LoadText():
    ...
    # with open("/Users/befeltingu/ML_research/thrush_processed.txt", "r") as text_file:
    #    data = text_file.read()

    text = open("/Users/befeltingu/NeuralNetworks/RNN/data/thrush.txt", "r").readlines()[0].split()
    # text = list(data)
    outputSize = len(text)
    data = list(set(text))
